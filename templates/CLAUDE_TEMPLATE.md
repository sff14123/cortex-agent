...
- **Locking**: 코드 수정 전 반드시 `relay.py status` 확인 및 `acquire`로 락을 획득하십시오. 종료 시 `pc_session_sync` 도구로 해제합니다.
...
- **Hashline Edit**: 코드 수정 시 절대 라인 번호에 의존하지 말고, `pc_strict_replace` 도구를 사용하여 원본과 100% 일치할 때 치환하십시오.
- **Evidence Based**: 작업 완료 주장 전 반드시 빌드/테스트 성공 증거(LSP, Exit 0 등)를 확보하십시오.
- **Anti-Hallucination**: `pc_memory_write` 도구 사용 시 `category: skill` 사용을 절대 금지합니다. (`insight`, `architecture` 등 사용)

## 3. 복잡도 기반 프로시저 (Pointers)
- **작업 계획 및 추적**: 복잡한 다단계 작업이나 터미널 간 전환 시, 임의로 진행하지 말고 먼저 Cortex 엔진을 가동하여 **MCP 도구(`pc_memory_read`)**로 `protocol::ultrawork` 및 `protocol::progress-tracking` 지식을 직접 조회하여 해당 절차를 따르십시오. (폭넓은 지식 및 스킬 검색이 필요한 경우 `pc_capsule` 도구를 사용하십시오)
